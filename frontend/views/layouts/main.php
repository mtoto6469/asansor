...
$url = Yii::$app->urlManager;
 $this->beginPage() ?>
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top  " id="min1">
        <div class="container-fluid ">
            <div class="navbar-header">
                <button type="button" class="navbar-toggle " data-toggle="collapse" data-target="#hid12" >
                ...
                if (!Yii::$app->user->isGuest) {
                    ...
                    <a class="navbar-brand page-scroll font_xs"
                       href="<?= $url->createAbsoluteUrl(['site/logout'], ['data-method' => 'post']) ?> ">خروج</a>
                    <a class="navbar-brand page-scroll"><?= $user->username ?></a>
                    <a class="navbar-brand page-scroll"
                       href="<?= $url->createAbsoluteUrl(['site/profile']) ?> ">پروفایل</a>

                    <?php
                } else {
                    ?>

                    <a class="navbar-brand page-scroll" href="<?= $url->createAbsoluteUrl(['site/signup']) ?>">ثبت
                        نام </a>
            ...
            <div class=" navbar-collapse  " id="bs-example-navbar-collapse-1">
                <ul class="nav navbar-nav navbar-right hid">
                    ...
                    <li class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown" >آموزش</a>
                        <ul class="dropdown-menu">
                            <?php $postcategory=\backend\models\Learn::find()->filterWhere(['enable_view'=>1])->andWhere(['enable'=>1])->all();
                            if ($postcategory != null){
                                foreach ($postcategory as $pc){
                                    ?>
                                    <li class=""><a href="<?= $url->createAbsoluteUrl(['site/learn','id'=>$pc->id])?>"><?= $pc->title;?></a>
                                        <?php
                                        //                                               $object=new pcat();
                                        //                                                $object->addpostcategory($pc->id);
                                        ?>
                ...
                <ul class="nav navbar-nav navbar-right hid1" id="hid12">
                    ...
                    <li class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown">پروژه ها</a>
                        <ul class="dropdown-menu">
                            <li><a class="page-scroll" href="<?= $url->createAbsoluteUrl(['site/projects']) ?>">پروژه
                                    های در دست اجرا </a></li>
                            <li><a class="page-scroll" href="<?= $url->createAbsoluteUrl(['site/project']) ?>">پروژه های
                                    پایان یافته</a></li>
                        </ul>
                    </li>
                    <li class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown">مقالات</a>
                            ...
                            if ($postcategory != null) {
                                foreach ($postcategory as $pc) {
                                    if ($pc->id_parent == 0) {
                                        ?>
                                        <li class="dropdown"><a class="dropdown-toggle"
                                                                href="<?= $url->createAbsoluteUrl(['site/post', 'id_parent' => $pc->id_parent, 'id' => $pc->id]) ?>"><?= $pc->title; ?></a>
                                        <?php
                                        $object = new pcat();
                                        $object->addpostcategory($pc->id);
                                    }
                    ...
                    <li><a class="page-scroll" href="<?= $url->createAbsoluteUrl(['site/']) ?>">آموزش</a>
                        <ul class="dropdown-menu">
                            <?php $postcategory = \frontend\models\Learn::find()->filterWhere(['enable_view' => 1])->andWhere(['enable' => 1])->all();
                            if ($postcategory != null) {
                                foreach ($postcategory as $pc) {
                                    ?>
                                    <li class=""><a
                                            href="<?= $url->createAbsoluteUrl(['site/learn', 'id' => $pc->id]) ?>"><?= $pc->title; ?></a>
                                        <?php
                                        $object = new pcat();
                                        $object->addpostcategory($pc->id);
                                        ?>
                    ...
                    <li><a class="page-scroll" href="<?= $url->createAbsoluteUrl(['massege/create']) ?>">درخواست فوری
                            تعمیرکار</a></li>
                    <li><a class="page-scroll" href="<?= $url->createAbsoluteUrl(['massege/create']) ?>">تماس با ما</a>
                    </li>
                    <li><a class="page-scroll" href="<?= $url->createAbsoluteUrl(['site/about']) ?>">درباره ما</a></li>
                    ...
                    <div class="col-lg-4 col-md-4">
                        ...
                        if ($foot_category != null) {
                            foreach ($foot_category as $footcat) {
                                ...
                                <ul>
                                    <li>
                                        <a href="<?= $url->createAbsoluteUrl(['tblcategory/cat', 'id_parent' => $footcat->id_parent, 'id' => $footcat->id]) ?>"><?= $footcat->name ?></a>
                                    </li>
                    ...
                    <div class="col-lg-4 col-md-4">
                        ...
                        if ($address) {
                            foreach ($address as $a) {
                                ?>
                                <p>آدرس شرکت :<?= $a->address ?></p>
                                <p> تلفن شرکت :<?= $a->phone ?></p>
                                <p> فکس شرکت :<?= $a->fax ?></p>
                                <p>موبایل :<?= $a->mobile ?></p>
                                <?php
                            }
        ...
        <div class="modal-dialog modal_zindex">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title font_l">استعلام قیمت انواع آسانسور</h4>
                </div>
                        ...
                        <ul>
                            ...
                            if ($floor != null) {
                                foreach ($floor as $f) {
                                    ?>
                                    <li>
                                        <a href="<?= $url->createAbsoluteUrl(['site/inquiry_price', 'floorNamber' => $f->floorNamber]) ?>"><?= $f->floorNamber ?>
                                            طبقه</a></li>
                                    <?php
                                }
                            }
                            ?>
                            <li><a href="<?= $url->createAbsoluteUrl(['massege/createhidro']) ?>">هیدرولیک</a></li>
                            <?php if (!Yii::$app->user->isGuest) {
                                $user = \common\models\User::findOne(Yii::$app->user->getId());
                                ?>
                                <li><a href="<?= $url->createAbsoluteUrl(['massege/create']) ?>">8 توقف به
                                        بالا(سایر)</a></li>
                                <?php
                            } else {
                                ...
                                ?>
                                <li><a href="<?= $url->createAbsoluteUrl(['site/login']) ?>">8 توقف به بالا(سایر)</a>
                                </li>
                            <?php } ?>
                        </ul>
                    </div>
...
class cat
{
    ...
    function addcategory($id_parent)
    {
        ...
        $cat = Tblcategory::find()->where(['id_parent' => $id_parent])->andWhere(['enable' => 1])->andWhere(['enable_view' => 1])->all();
//        var_dump($cat);exit();
        ?>
...
class pcat
{
    ...
    {
        $url = Yii::$app->urlManager;
        $pcat = \backend\models\Tblpostcategory::find()->where(['id_parent' => $id_parent])->andWhere(['enable' => 1])->andWhere(['enable_view' => 1])->all(); ?>
        <ul class="tt-dropdown-menu sub_manu">
...
<div class="">
    ...
    <script src="<?php Yii::$app->request->hostInfo ?>/frontend/web/js/vendor/jquery-1.11.2.min.js"></script>
    <script src="<?php Yii::$app->request->hostInfo ?>/frontend/web/js/jquery.mixitup.min.js"></script>
    <script src="<?php Yii::$app->request->hostInfo ?>/frontend/web/js/plugins.js"></script>
    <script src="<?php Yii::$app->request->hostInfo ?>/frontend/web/js/main.js"></script>
